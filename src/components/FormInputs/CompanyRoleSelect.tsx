...
import { CompanyRoleFragment, CompanyRoleWhereInput, PaginateCompanyRolesDocument } from 'src/graphql';
...
export const CompanyRoleSelect: React.FC<SelectProps> = (props) => {
  ...
  const handleData = (items: CompanyRoleFragment[]) => {
  ...
  useEffect(() => {
    if (!loading && data?.paginateCompanyRoles?.items?.length && !props.value) {
      const firstRole = data.paginateCompanyRoles.items[0];
  ...
  return (
    <Select
      ...
      data={handleData(data?.paginateCompanyRoles.items ?? [])}
    />
  );
};
